...
#
# Example 3: forward-facing step

FDOX   = 1            # Order of approximation (Numerical scheme)
FDOY   = 64           # Order of approximation (Numerical scheme)
...
dywall = 0.0133       # First point in the normal direction
...
x = np.linspace(-5*X0, Xf, 100*Nx)
y = 1.25e-3*(-1-np.tanh(10*(x-2*X0)))
coords = np.vstack([x.reshape((1,100*Nx)), y.reshape(1,100*Nx)]).T

# Domain for the N-S computations
GeomNS = Geom(coords, Lref, Nx, Ny, Y0=0, Yf=Yf, dywall=dywall, Optimal=True, Smooth=250,opt=0.1)

# For the PSE, the solution is interpolated from the NS results
# Start at the reference Reynolds (X=1)
x = np.linspace(1, Xf, 100*Nx) ; y = 1.25e-3*(-1-np.tanh(10*(x-2*X0)))
coords = np.vstack([x.reshape((1,100*Nx)), y.reshape(1,100*Nx)]).T

# Domain for the PSE computations
# 512 points in x
# 6 elements in y (order 64, so 384 points in y)
# dywall size of first element
# Smooth the position of the 
GeomPSE = Geom(coords, Lref, 512, 6, Y0=0, Yf=300, dywall=20, Smooth=500, Optimal=True)

# Numerical methods set-up
NumMethod = NumericalMethod(GeomNS, GeomPSE, FDOX=FDOX, FDOY=FDOY, stabCoeff=1, tol=1e-4)
NumMethod.SetUp()
...
setParam = Parameter(M,N, NumMethod.PSE.X1D, maxN=N, MFD=True)

#  * * *  Flow properties  * * *
# Reference length
setParam.dic['prop']['Lref']  = Lref
# Reference Mach Number
setParam.dic['prop']['Ma']    = 0.5
...
# I/O
setParam.dic['io']['LoadName']   = '../_results/example3'
setParam.dic['io']['SaveName']   = '../_results/example3'
...
for m, n in setParam.dic['modes'][0]:
    ...
    if n != 0:
        setParam.dic[(m,n)]['A_0'] = Amplitude * 1/(n+1)
...
for i in range(1, NumMethod.PSE.Nx):
    for m,n in [(i,j) for j in range(N) for i in range(M)]:
        ...
        setParam.dic[(m,n)]['linear'][i]   = False
...
# Plotting graph
plotStuff.plotAll(Param, Flow, Fluc, Igraph, dispGrid=True, stabilityMap=False)
