...
FDOX   = 3            # Order of approximation (Numerical scheme)
FDOY   = 5            # Order of approximation (Numerical scheme)
...
dywall = 0.02         # First point in the normal direction
Yf     = 500          # Last point in the normal direction 
...
x = np.linspace(-5*X0, Xf, Nx)
y = np.zeros_like(x)

# Coordinates
coords = np.vstack([x.reshape((1,Nx)), y.reshape(1,Nx)]).T
Geom = Geom(coords, Lref, Nx, Ny, Y0=0, Yf=Yf, dywall=dywall)
NumMethod = NumericalMethod(Geom, FDOX=FDOX, FDOY=FDOY, tol=1e-6)
NumMethod.SetUp()

# Number of modes (M,N)
# Krypton can be used for crossflow instabilities, but
# this option was never tested
# M is the number of modes in the span-wise direction
M  = 1
dF = 25
# First mode
F0 = 75
...
setParam = Parameter(M,N,NumMethod.PSE.Nx)

#  * * *  Flow properties  * * *
# Reference length
setParam.dic['prop']['Lref']  = Lref
# CrossFlow angle (degree)
setParam.dic['prop']['CF']    = 0
# Wall temperature
setParam.dic['prop']['Twall type'] = 'constant'
# Twall
setParam.dic['prop']['Twall'] = Twall  
# Reference Mach Number
setParam.dic['prop']['Ma']    = 0.1
# Viscosity is variable by default (Sutherland), but Constant is also implemented
setParam.dic['prop']['viscosity'] = 'Sutherland'
# Local/reference Reynolds Number
setParam.dic['prop']['Rex']   = Re0*np.sign(NumMethod.PSE.X1D)*((abs(NumMethod.PSE.X1D)*Lref))**0.5
i0 = np.argmin(abs(setParam.dic['prop']['Rex']-Re0))
setParam.dic['prop']['Re']    = setParam.dic['prop']['Rex'][i0]
# Non-dimensional frequencies
setParam.dic['prop']['F']     = F0
setParam.dic['prop']['dF']    = dF
setParam.dic['prop']['B']     = 0.
setParam.dic['prop']['dB']    = 0.

# I/O
setParam.dic['io']['LoadName']   = '../_results/testCase'
setParam.dic['io']['SaveName']   = '../_results/testCase'
...
for m, n in setParam.dic['modes'][i0]:
    setParam.dic[(m,n)]['local'][i0]    = True
    setParam.dic[(m,n)]['linear'][i0]   = True
    setParam.dic[(m,n)]['parallel'][i0] = True
    setParam.dic[(m,n)]['A_0'] = 1

# And then the other stations
for i in range(i0+1, NumMethod.PSE.Nx):
    for m,n in [(i,j) for j in range(N) for i in range(M)]:
        setParam.dic[(m,n)]['local'][i]    = False
        setParam.dic[(m,n)]['linear'][i]   = True
        setParam.dic[(m,n)]['parallel'][i] = True
...
time_ff1 = time.time()
Fluc = FlucFlow(NumMethod.PSE, Param, Flow, i0=i0, ipse=i0)
time_ff2 = time.time()
...
Igraph = [i0, NumMethod.PSE.Nx-1]

# Plotting graph
plotStuff.plotAll(Param, Flow, Fluc, Igraph, i0=i0, example=1)
